...
$(document).ready(function() {
    ...
    function validateItem(obj) {
        var flag = true;
        $('.errMsz').remove();
        $(obj).prev('ul').find('.req').each(function(i, option) {
                ...
                if (dateStatus.length != 3) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (!record.success) {
                                $(me).find('input').after('<span class="errMsz">Invalid Date</span>');
                                flag = false;
                            }
                            else {
                                saveDetails(obj, currentselected.toLowerCase());
                                createLayout();
                                closeSubItem();
                                $(obj).remove();
                            }
...
function createLayout() {
    ...
    $.each(detailArr, function(i, option) {
        ...
        if (option.id.split("-")[1] === 'bank') {
            ...
            insert += '<span class="bedate blast-row">Expire On : ' + option.data[8].value + '</span>';
            insert += '</li>';
            insert += '</ul>';
            insert += '</div>';
        }
        if (option.id.split("-")[1] === 'passport') {
            ...
            insert += '<span class="pidate pfirst-row">Isuue date :</span>';
            insert += '<span class="pidate pfirst-row">' + option.data[2].value + '</span>';
            insert += '<span class="pedate psecond-row">Expire Date :</span>';
